...
var pt = {
	...
	init: function() {
		...
		sys.onload(function() {
			pt.err_m = sys.$("err_m");
			if (g_appid != 1003903) {
				document.body.onclick = function(Q) {
					Q = Q | window.event;
					pt.action[0]++
				}
			}
			document.body.onkeydown = function(Q) {
				Q = Q | window.event;
				pt.action[1]++
			};
			...
			window.setTimeout(function() {
				ptui_reportAttr2("258144&union=256038", 0.05)
			},
			1000);
			try {
				var I = sys.getQueryValue("s_url");
				var F = sys.getQueryValue("style");
				var C = sys.getQueryValue("appid");
				var D = /paipai.com$/.test(window.location.hostname);
				var M = sys.getQueryValue("regmaster");
				if (F == 5) {
					var G = /\?/g.test(I) ? "&": "?";
					G += "login2qq=1&webqq_type=10";
					I += G
				}
				var K = sys.getQueryValue("hide_close_icon") == 1 || sys.getQueryValue("hide_title_bar") == 1;
				var E = document.createElement("h4");
				E.innerHTML = '<input type="button" class="btn_close" id="close" name="close" onclick="javascript:onPageClose();" title="关闭" /><u id="label_login_title">用户登录</u>';
				if (window.g_href && location.href.indexOf(g_href) == - 1 && /^ui.ptlogin2./.test(location.hostname)) {
					if (location.protocol == "http:") {
						if (window.g_jumpname == "aqjump" && F != 5) {
							if (!K && ! sys.$("close")) {
								sys.$("login").insertBefore(E, sys.$("normal_login"));
								sys.$("login").style.border = "1px"
							}
							var P = document.createElement("div");
							P.style.textAlign = "center";
							P.innerHTML = '<div style="position:relative;">							<br/>							<p style="padding-bottom:20px;">您的网络存在不安全因素，请使用安全登录。</p></div>							<input id="safe_login" value="安全登录"" type="button" class="btn" style="text-align:center;"/>							</div>							<div style="margin-top:10px;margin-left:10px; height:20px;">							<span style="float:left;height:15px;width:14px; background: url(https://ui.ptlogin2.qq.com/style/14/images/help.png) no-repeat scroll right center transparent;"></span>							<a style="float:left; margin-left:5px;" href="http://kf.qq.com/info/80861.html" target="_blank" >什么是不安全因素</a>							</div>';
							sys.$("loginform").style.display = "none";
							sys.$("web_login").appendChild(P);
							ptui_notifySize("login");
							ptui_reportAttr2(245663);
							var O = new Image();
							var N = encodeURIComponent(window.g_href + "|_|" + location.href + "|_|" + window.g_jumpname + "|_|mid=245663");
							O.src = "http://badjs.qq.com/cgi-bin/js_report?bid=110&msg=" + N + "&v=" + Math.random()
						} else {
							var O = new Image();
							var N = encodeURIComponent(window.g_href + "|_|" + location.href + "|_|" + window.g_jumpname + "|_|mid=245580");
							O.src = "http://badjs.qq.com/cgi-bin/js_report?bid=110&mid=245580&msg=" + N + "&v=" + Math.random()
						}
					} else {
					...
					if (sys.$("safe_login")) {
						sys.$("safe_login").onclick = function() {
							...
							var Q = "https://ui.ptlogin2.qq.com/cgi-bin/login";
							window.open(Q + "?style=14&pt_safe=1&appid=" + C + "&s_url=" + I + "&regmaster=" + M, "_top");
					...
					document.forms[0].u1.value = I;
					var H = (g_jumpname == "jump" || g_jumpname == "") ? encodeURIComponent("u1=" + encodeURIComponent(document.forms[0].u1.value)) : "";
					sys.$("xui") && (sys.$("xui").src = sys.$("xui").src + "&jumpname=" + g_jumpname + "&param=" + H + "&qtarget=" + L)
				}
			} catch(J) {}
		});
	...
	get_qrlogin_pic: function() {
		...
		A += "appid=" + g_appid + "&e=2&l=M&s=3&d=72&v=4&t=" + Math.random();
		return A
	},
	animate: function(B, C, H, R, G) {
		...
		var A = function() {
			var U = true;
			for (var V = 0; V < O; V++) {
				F[D[V]] = F[D[V]] + J[D[V]] * Math.abs(C[D[V]] - Math.floor(F[D[V]]) * H / 100);
